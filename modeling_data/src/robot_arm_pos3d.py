...
model = load_model_from_path("./assets/robot_arm_3d.xml")
...
for idx, value in enumerate(lookat):
    viewer.cam.lookat[idx] = value
viewer2.cam.distance = 3
viewer2.cam.azimuth = 0.
viewer2.cam.elevation = 30.
...
def mujoco_human():
    while 1:
        ...
        if np.linalg.norm(sim.data.site_xpos[grasp_id] - sim.data.body_xpos[target_id])< 0.01:
            # print("done")
            # pos = np.random.rand(2)
        # sim.data.site_xpos[target_id] = pos
        # sim.model.site_pos[target_id] = pos

            random_pos = (np.random.rand(3) - 0.5) * 0.4
            sim.data.qpos[0:3] = random_pos
            sim.data.qpos[3:6] = random_pos + (np.random.rand(3) - 0.5) * 0.2
        ...
        pos = pos1 + pos2
        value = [pos[0], -pos[2] + 0.3, pos[1] + 0.1]

        sim.data.set_mocap_pos("mocap", value)
...
def mujoco_auto():
    ...
    path1 = "./modeling_ur/data/auto/current.txt"
    path2 = "./modeling_ur/data/auto/target.txt"
    path3 = "./modeling_ur/data/auto/observation.txt"
    ...
    while 1:
        ...
        pos = [sim.data.mocap_pos[0][0] + delta_pos[0], sim.data.mocap_pos[0][1] + delta_pos[1], sim.data.mocap_pos[0][2] + delta_pos[2]]
        # sim.data.set_mocap_pos("mocap", pos)
        sim.data.set_mocap_pos("mocap", sim.data.body_xpos[obs_id])
...
def mujoco_shared():
    ...
    while 1:
        ...
        if np.linalg.norm(sim.data.site_xpos[grasp_id] - sim.data.body_xpos[target_id])< 0.02:
        ...
        if np.linalg.norm(sim.data.site_xpos[grasp_id] - sim.data.body_xpos[obs_id]) > 0.01 and auto_mode == True:
            ...
            delta_pos = -(sim.data.site_xpos[grasp_id] - sim.data.body_xpos[obs_id]) * 0.004
